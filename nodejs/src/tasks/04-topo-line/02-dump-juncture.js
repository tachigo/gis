...
(async () => {
  ...
  await $.Utils.call(`外国 外圈陆地边界线`, async () => {
    ...
    for await (const item of list) {
      ...
      await $.Utils.call(`${name}#${id} 外圈陆地边界线`, LibTopology.dumpJunctureAvgVertices.bind(LibTopology), [pg, id, 'outer']);
    }
  });
  ...
  await $.Utils.call(`外国 内圈陆地边界线`, async () => {
    ...
    for await (const item of list) {
      const id = +item['id'];
      const name = item['name'];
      await $.Utils.call(`${name}#${id} 内圈陆地边界线`, LibTopology.dumpJunctureAvgVertices.bind(LibTopology), [pg, id, 'inner']);
    }
  });
